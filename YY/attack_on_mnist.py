...
from tools import evaluate, predict, pseudorandom_target, load_mnist, out_, sample_img, exclude_miss
...
def make_adv(sess, env, X_data, y_data):
    ...
    name_out = 'MNIST_'+str(eps)+'_'+str('targeted' if env.targeted else 'untargeted')+'_YY'
    ...
    with open(path_out, mode='w') as f:
        ...
        f.write('mnist_YY : eps='+str(eps)+' <'+str('targeted' if env.targeted else 'untargeted')+'>\n')
...
env.saver.restore(sess, 'models/model_mnist/{}'.format('model_mnist'))
...
sample_img(sess, env, X_adv, X_test, y_test, name='mnist')
out_(sess, env, X_adv, X_test, y_test, name='mnist', attack='YY')
